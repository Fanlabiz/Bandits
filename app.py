    ...
    def calculate_culture_in_radius(self, x: int, y: int, length: int, width: int, 
        ...
        for i in range(max(0, center_x - radius), 
            ...
            for j in range(max(0, center_y - radius),
                ...
                # CORRECTION: Vérifier que c'est bien un bâtiment placé ( > 0)
                if cell_value > 0:
                    # CORRECTION: Vérifier que l'indice est valide
...
if uploaded_file and optimize_button:
    try:
        ...
        if any(results['boost_stats'].values()):
            ...
            st.info("ℹ️ Aucun boost détecté - les seuils sont peut-être trop élevés")
