...
def create_colorful_heatmap(grid: np.ndarray, placed_buildings: List[dict], title: str):
    """
    Crée une visualisation colorée avec chaque bâtiment d'une couleur différente
    """
    ...
    for i in range(grid.shape[0]):
        ...
        for j in range(grid.shape[1]):
            val = grid[i, j]
            if val == -1:
                row.append('rgb(64,64,64)')  # Gris foncé pour obstrué
            elif val == 0:
                row.append('rgb(255,255,255)')  # Blanc pour libre
            else:
                # Couleur basée sur l'ID du bâtiment
    ...
    for placed in placed_buildings:
        ...
        center_x = y + placed['width'] // 2
        center_y = x + placed['length'] // 2
        
        # Couleur selon le boost
        if placed['building'].is_producer():
            ...
            if placed['boost'] >= 2.0:
                bg_color = 'gold'
                border = 'darkgoldenrod'
            elif placed['boost'] >= 1.5:
                bg_color = 'lightgreen'
                border = 'darkgreen'
            elif placed['boost'] >= 1.25:
                bg_color = 'lightblue'
                border = 'darkblue'
            else:
                bg_color = 'lightgray'
                border = 'gray'
            text = placed['building'].name[:3]
...
if uploaded_file and optimize_button:
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            # Exporter le tableau récapitulatif (avec TOUS les bâtiments)
        ...
        st.download_button(
            label="📥 Télécharger les résultats (Excel simplifié)",
            data=output.getvalue(),
            file_name="resultats_complets.xlsx"
        )
