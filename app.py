...
class BuildingPlacer:
    ...
    def calculate_boost_for_building(self, building: Building, x: int, y: int, 
        ...
        total_culture = self.calculate_culture_in_radius(x, y, length, width, building.radius)
    ...
    def place_building(self, building: Building, x: int, y: int, orientation: str) -> bool:
        ...
        initial_boost = self.calculate_boost_for_building(building, x, y, length, width)
    ...
    def calculate_final_production(self) -> Tuple[float, dict]:
        """
        ...
        for placed in self.placed_buildings:
            ...
            production = building.culture_produced if building.is_producer() else 0
            total_production += production  # Les bâtiments non-producteurs n'ajoutent rien à la production
        ...
        return total_production, boost_stats
            ...
            for x in range(min_x, max_x - length + 2):
                for y in range(min_y, max_y - width + 2):
                    if self.can_place_in_zone(zone, x, y, length, width):
                        ...
                        score = boost * 100 + proximity_bonus
                        
                        if score > best_score:
                            best_score = score
                            best_placement = {'x': x, 'y': y, 'orientation': orientation, 'boost': boost}
    ...
    def place_all_buildings(self) -> dict:
        ...
        consumers.sort(key=lambda b: (b.boost_25, b.boost_50, b.boost_100))
        ...
        for consumer in all_consumers:
            ...
            for zone_idx, zone in enumerate(self.available_zones):
                if consumer.get_dimensions('H')[0] * consumer.get_dimensions('H')[1] <= len(zone):
        ...
        total_production, boost_stats = self.calculate_final_production()
        
        return {
            'total_production': total_production,
            'buildings_placed': len(self.placed_buildings),
            'buildings_total': total_to_place,
            'buildings_remaining': len(remaining),
...
st.markdown("""
**Configuration :**
- **Bâtiments producteurs** (avec culture > 0 et rayonnement > 0) : placent de la culture pour booster les autres
    ...
    try:
        ...
        for _, row in df_buildings.iterrows():
            building = Building(
                ...
                quantity=int(row['quantité']) if not pd.isna(row['quantité']) else 1,
                culture_produced=float(row['culture']) if not pd.isna(row['culture']) else 0,
                radius=int(row['rayonnement']) if not pd.isna(row['rayonnement']) else 0,
                boost_25=float(row['Boost 25%']) if not pd.isna(row['Boost 25%']) else 0,
                boost_50=float(row['Boost 50%']) if not pd.isna(row['Boost 50%']) else 0,
                boost_100=float(row['Boost 100%']) if not pd.isna(row['Boost 100%']) else 0
            )
