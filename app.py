...
if uploaded_file is None:
    ...
    try:
        with st.spinner("📊 Analyse du fichier..."):
            # CORRECTION 1: Spécifier le moteur openpyxl explicitement
            ...
            # CORRECTION 2: Afficher les colonnes pour debug
            st.write("📋 Colonnes trouvées dans votre fichier:", list(df_buildings.columns))
            
            # CORRECTION 3: Mapping flexible des noms de colonnes
            buildings = []
            for idx, row in df_buildings.iterrows():
                # Fonction pour récupérer une valeur avec différents noms possibles
                ...
                # Adapter ces listes selon vos noms de colonnes réels
                building = Building(
                    name=str(get_value(['Nom', 'nom', 'NAME', 'Batiment', 'bâtiment'], f"Bâtiment_{idx}")),
                    length=int(get_value(['longueur', 'Longueur', 'LONGUEUR', 'len', 'length'], 1)),
                    width=int(get_value(['largeur', 'Largeur', 'LARGEUR', 'width'], 1)),
                    quantity=int(get_value(['quantité', 'Quantité', 'QUANTITE', 'qty', 'quantity'], 1)),
                    culture=float(get_value(['culture', 'Culture', 'CULTURE', 'prod', 'production'], 0)),
                    radius=int(get_value(['rayonnement', 'Rayonnement', 'RAYONNEMENT', 'radius'], 1)),
                    boost_25=float(get_value(['Boost 25%', 'boost_25', 'BOOST_25', 'Boost25'], 0)),
                    boost_50=float(get_value(['Boost 50%', 'boost_50', 'BOOST_50', 'Boost50'], 0)),
                    boost_100=float(get_value(['Boost 100%', 'boost_100', 'BOOST_100', 'Boost100'], 0))
                )
                buildings.append(building)
            
            st.success(f"✅ Fichier chargé avec succès! {len(buildings)} types de bâtiments trouvés.")
            ...
            with col1:
                st.metric("🏢 Bâtiments placés", 
                         ...
                         f"Total demandé: {sum(b.quantity for b in buildings)}")
            with col2:
        ...
        **Conseils de débogage:**
        ...
        2. Vérifiez les noms des colonnes dans l'onglet 'Bâtiments'
        3. Assurez-vous que l'onglet 'Terrain' ne contient que des 0 et 1
        """)
